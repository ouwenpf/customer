...
create view accoding_stock_dif_bak as select `acc_base`.`FRelateBrID` AS `FRelateBrID`,concat('LH',`acc_base`.`FStockID`) AS `FStockID`,`acc_base`.`cate_id` AS `FItemID`,`uctoo_lvhuan`.`uct_cate_account`.`account_num` AS `FdifQty`,date_format(from_unixtime(`acc_base`.`FdifTime`),'%Y-%m-%d %H:%i:%s') AS `FdifTime` from (((select `uctoo_lvhuan`.`uct_cate_account`.`branch_id` AS `FRelateBrID`,`uctoo_lvhuan`.`uct_cate_account`.`warehouse_id` AS `FStockID`,`uctoo_lvhuan`.`uct_cate_account`.`cate_id` AS `cate_id`,max(`uctoo_lvhuan`.`uct_cate_account`.`createtime`) AS `FdifTime` from `uctoo_lvhuan`.`uct_cate_account` group by `uctoo_lvhuan`.`uct_cate_account`.`cate_id`,`uctoo_lvhuan`.`uct_cate_account`.`warehouse_id`)) `acc_base` join `uctoo_lvhuan`.`uct_cate_account`) where ((`uctoo_lvhuan`.`uct_cate_account`.`cate_id` = `acc_base`.`cate_id`) and (`uctoo_lvhuan`.`uct_cate_account`.`createtime` = `acc_base`.`FdifTime`) and (`uctoo_lvhuan`.`uct_cate_account`.`warehouse_id` = `acc_base`.`FStockID`));
...
create view cuplus_select as select `tst`.`id` AS `id`,concat('#',`pur`.`order_id`) AS `order_id`,`pur`.`branch_id` AS `branch_id` from (`uctoo_lvhuan`.`table_selecttool` `tst` join `uctoo_lvhuan`.`uct_waste_purchase` `pur`) where (`tst`.`id` = `pur`.`id`);
...
create view view_cargofile_first as select `uctoo_lvhuan`.`uct_waste_cate`.`id` AS `id`,`uctoo_lvhuan`.`uct_waste_cate`.`name` AS `name` from `uctoo_lvhuan`.`uct_waste_cate` where (`uctoo_lvhuan`.`uct_waste_cate`.`parent_id` = 0);
...
create view view_excel_customer as select `uctoo_lvhuan`.`uct_admin`.`id` AS `id`,`uctoo_lvhuan`.`uct_admin`.`nickname` AS `nickname` from `uctoo_lvhuan`.`uct_admin` where (`uctoo_lvhuan`.`uct_admin`.`id` > 1826);
...
create view view_valid_sell as select `uctoo_lvhuan`.`uct_waste_sell_log`.`sell_id` AS `sell_id` from `uctoo_lvhuan`.`uct_waste_sell_log` where ((`uctoo_lvhuan`.`uct_waste_sell_log`.`state_value` = 'wait_confirm_order') and (`uctoo_lvhuan`.`uct_waste_sell_log`.`state_text` = '待付款'));
...
create view view_warehouse_collect as select `uctoo_lvhuan`.`uct_waste_warehouse`.`id` AS `id`,`uctoo_lvhuan`.`uct_waste_warehouse`.`name` AS `name` from `uctoo_lvhuan`.`uct_waste_warehouse` where (`uctoo_lvhuan`.`uct_waste_warehouse`.`parent_id` = 0);




CREATE OR REPLACE VIEW accoding_stock AS
SELECT
...
CREATE  VIEW trans_daily_sel AS 
SELECT 
...
CREATE VIEW trans_daily_wh_profit AS
SELECT
    "uctoo_lvhuan"."trans_main_table"."FRelateBrID" AS "FRelateBrID",
...
CREATE VIEW trans_daily_sor AS
SELECT
...
CREATE VIEW trans_finishcount AS 
SELECT 
    uctoo_lvhuan.trans_main_table.FRelateBrID AS FRelateBrID,
    uctoo_lvhuan.trans_main_table.FDeptID AS FDeptID,
    CASE 
...
SELECT 
    ...
    CASE uctoo_lvhuan.trans_log_table.TpurchaseOver 
        ...
        ELSE DATE_FORMAT(FROM_UNIXTIME(uctoo_lvhuan.trans_log_table.Tpurchase), '%Y-%m-%d') 
    END AS Date,
    ...
    (CASE uctoo_lvhuan.uct_waste_purchase.hand_mouth_data WHEN '1' THEN '1' ELSE '0' END) + 
        (CASE uctoo_lvhuan.uct_waste_purchase.give_frame WHEN '1' THEN '3' ELSE '0' END) AS FSaleStyle,
...
FROM 
    ...
    JOIN uctoo_lvhuan.uct_waste_customer ON uctoo_lvhuan.uct_waste_purchase.customer_id = uctoo_lvhuan.uct_waste_customer.id) 
    JOIN uctoo_lvhuan.trans_log_table ON uctoo_lvhuan.uct_waste_purchase.id = uctoo_lvhuan.trans_log_table.FInterID) 
    LEFT JOIN uctoo_lvhuan.trans_total_fee_rf ON trans_total_fee_rf.purchase_id = uctoo_lvhuan.uct_waste_purchase.id) 
WHERE 
    uctoo_lvhuan.trans_log_table.FTranType = 'PUR' 
    AND uctoo_lvhuan.uct_waste_purchase.order_id > '201806300000000000'

UNION ALL

SELECT 
    uctoo_lvhuan.uct_waste_sell.branch_id AS FRelateBrID,
    uctoo_lvhuan.uct_waste_sell.id AS FInterID,
    'SEL' AS FTranType,
    CASE uctoo_lvhuan.trans_log_table.TallowOver 
        WHEN '0' THEN DATE_FORMAT(FROM_UNIXTIME(1), '%Y-%m-%d %H:%i:%S') 
        ELSE DATE_FORMAT(FROM_UNIXTIME(uctoo_lvhuan.trans_log_table.Tallow), '%Y-%m-%d %H:%i:%S') 
    END AS FDate,
    CASE uctoo_lvhuan.trans_log_table.TallowOver 
        WHEN '0' THEN DATE_FORMAT(FROM_UNIXTIME(1), '%Y-%m-%d') 
        ELSE DATE_FORMAT(FROM_UNIXTIME(uctoo_lvhuan.trans_log_table.Tallow), '%Y-%m-%d') 
    END AS Date,
    '1' AS FTrainNum,
    uctoo_lvhuan.uct_waste_sell.order_id AS FBillNo,
    uctoo_lvhuan.uct_waste_sell.customer_id AS FSupplyID,
    NULL AS Fbusiness,
    CONCAT('DC', uctoo_lvhuan.uct_waste_sell.customer_id) AS FDCStockID,
    CASE WHEN uctoo_lvhuan.uct_waste_sell.purchase_id IS NOT NULL THEN CONCAT('LH', uctoo_lvhuan.uct_waste_sell.warehouse_id) ELSE '' END AS FSCStockID,
    CASE uctoo_lvhuan.uct_waste_sell.state 
        WHEN 'cancel' THEN '0'
        ELSE '1'
    END AS FCancellation,
    '0' AS FROB,
    ...
    '' AS FExplanation,
    '4' AS FDeptID,
    uctoo_lvhuan.uct_waste_sell.seller_id AS FEmpID,
    uctoo_lvhuan.trans_log_table.TcheckPerson AS FCheckerID,
    CASE uctoo_lvhuan.trans_log_table.TcheckOver
        WHEN '0' THEN NULL
        ELSE DATE_FORMAT(FROM_UNIXTIME(uctoo_lvhuan.trans_log_table.Tcheck), '%Y-%m-%d %H:%i:%S') 
    END AS FCheckDate,
    uctoo_lvhuan.uct_waste_sell.customer_linkman_id AS FFManagerID,
    uctoo_lvhuan.uct_waste_sell.customer_linkman_id AS FSManagerID,
    uctoo_lvhuan.uct_waste_sell.seller_id AS FBillerID,
    '1' AS FCurrencyID,
    uctoo_lvhuan.trans_log_table.state AS FNowState,
    CASE WHEN uctoo_lvhuan.uct_waste_sell.purchase_id IS NOT NULL THEN '1' ELSE '2' END AS FSaleStyle,
    NULL AS FPOStyle,
    NULL AS FPOPrecent,
    ROUND(uctoo_lvhuan.uct_waste_sell.cargo_weight, 1) AS TalFQty,
    ROUND(uctoo_lvhuan.uct_waste_sell.cargo_price, 2) AS TalFAmount,
    uctoo_lvhuan.uct_waste_sell.materiel_price AS TalFeeFrist,
    uctoo_lvhuan.uct_waste_sell.other_price AS TalFeeSecond,
    0 AS TalFeeThird,
    0 AS TalFeeForth,
    0 AS TalFeeFifth 
FROM 
    (((uctoo_lvhuan.uct_waste_sell 
    JOIN uctoo_lvhuan.uct_waste_customer ON uctoo_lvhuan.uct_waste_sell.customer_id = uctoo_lvhuan.uct_waste_customer.id) 
    JOIN uctoo_lvhuan.trans_log_table ON uctoo_lvhuan.uct_waste_sell.id = uctoo_lvhuan.trans_log_table.FInterID) 
    JOIN uctoo_lvhuan.uct_waste_purchase ON uctoo_lvhuan.uct_waste_sell.purchase_id = uctoo_lvhuan.uct_waste_purchase.id) 
WHERE 
    uctoo_lvhuan.trans_log_table.FTranType = 'SEL' 
    AND uctoo_lvhuan.uct_waste_sell.purchase_id IS NULL 
    AND uctoo_lvhuan.uct_waste_sell.order_id > '201806300000000000';
    
    
    
    

CREATE VIEW trans_month_customer AS 
SELECT 
...
CREATE VIEW trans_month_sel AS
SELECT
...
FROM
    (SELECT
        ...
        GROUP_CONCAT(FItemID SEPARATOR ',') AS FItemID,
        ROUND(SUM(trans_assist_table.FQty), 1) AS total_weight,
        ROUND(SUM(trans_assist_table.FAmount), 2) AS total_price,
        COUNT(DISTINCT trans_assist_table.FinterID) AS count_order,
        DATE_FORMAT(trans_main_table.FDate, '%Y-%m-%d') AS FDate
    FROM
        (trans_main_table
    JOIN trans_assist_table ON trans_main_table.FInterID = trans_assist_table.FinterID)
    WHERE
        trans_main_table.FTranType = 'SEL'
        AND trans_main_table.FTranType = trans_assist_table.FTranType
        AND trans_main_table.FStatus = 1
        AND trans_main_table.FCancellation = 1
    GROUP BY
        trans_main_table.FRelateBrID,
        DATE_FORMAT(trans_main_table.FDate, '%Y-%m-%d')
    ) monsel
WHERE
    NOT monsel.FDate IN (SELECT DATE_FORMAT(FDate, '%Y-%m-%d') FROM trans_month_sel_table);




CREATE VIEW trans_month_sel_rank AS
SELECT
    sel_rank.FRelateBrID AS FRelateBrID,
    sel_rank.FItemID AS FItemID,
    sel_rank.total_weight AS total_weight,
    sel_rank.total_price AS total_price,
    sel_rank.FDate AS FDate
FROM
    (SELECT
        trans_main.FRelateBrID AS FRelateBrID,
        trans_assist.FItemID AS FItemID,
        ROUND(SUM(trans_assist.FQty), 1) AS total_weight,
        ROUND(SUM(trans_assist.FAmount), 2) AS total_price,
        DATE_FORMAT(trans_main.FDate, '%Y-%m-%d') AS FDate
    FROM
        trans_main
    JOIN trans_assist ON trans_main.FInterID = trans_assist.FInterID
    WHERE
        trans_main.FTranType = 'SEL'
        AND trans_assist.FTranType = 'SEL'
        AND trans_main.FStatus = 1
    GROUP BY
        trans_main.FRelateBrID,
        DATE_FORMAT(trans_main.FDate, '%Y-%m-%d'),
        trans_assist.FItemID
    ) sel_rank
WHERE
    NOT sel_rank.FDate IN (SELECT DATE_FORMAT(FDate, '%Y-%m-%d') FROM trans_month_sel_rank_table);
...
CREATE VIEW trans_month_sor AS
SELECT
    trans_main_table.FRelateBrID AS FRelateBrID,
...
CREATE VIEW trans_month_wh_profit AS
SELECT
...
CREATE VIEW trans_ordercount AS
SELECT
    `uctoo_lvhuan`.`trans_main_table`.`FRelateBrID` AS `FRelateBrID`,
    `uctoo_lvhuan`.`trans_main_table`.`FDeptID` AS `FDeptID`,
    `uctoo_lvhuan`.`trans_main_table`.`FEmpID` AS `FEmpID`,
...
CREATE VIEW trans_total_cargo AS
SELECT
    `uctoo_lvhuan`.`uct_waste_purchase_cargo`.`purchase_id` AS `FInterID`,
    'PUR' AS `FTranType`,
...
CREATE VIEW trans_total_cargo_ch AS
SELECT
    tctable.FInterID AS FInterID,
    tctable.FQtyTotal AS PUR_FQtyTotal,
    tctable.FAmountTotal AS PUR_FAmountTotal,
    innercargo.FQtyTotal AS SS_FQtyTotal,
    innercargo.FAmountTotal AS SS_FAmountTotal
FROM
    (
        SELECT
            tctable.FSourceInterID::integer AS FInterID, -- Explicit cast to integer
            tctable.FQtyTotal AS FQtyTotal,
            tctable.FAmountTotal AS FAmountTotal
        FROM
            uctoo_lvhuan.trans_total_cargo_table tctable
        WHERE
            tctable.FTranType = 'SEL' AND tctable.FSourceInterID > '0' -- Compare with string '0'
        UNION ALL
        SELECT
            tctable.FInterID AS FInterID,
            tctable.FQtyTotal AS FQtyTotal,
        ...
        FROM
            uctoo_lvhuan.trans_total_cargo_table tctable
        WHERE
            tctable.FTranType = 'SOR'
    ) innercargo
JOIN uctoo_lvhuan.trans_total_cargo_table tctable ON tctable.FTranType = 'PUR' AND tctable.FInterID = innercargo.FInterID;



CREATE VIEW trans_total_cargo_sec AS
SELECT
    uctoo_lvhuan.uct_waste_sell_cargo.sell_id AS FInterID,
    'SEL' AS FTranType,
    SUM(uctoo_lvhuan.uct_waste_sell_cargo.net_weight) AS FQtyTotal,
...
CREATE VIEW trans_total_fee AS 
SELECT 
...
CREATE  VIEW view_cargofile_collect AS 
SELECT 
    view_cargofile_third.id AS id,
    view_cargofile_first.name AS first_sort,
    view_cargofile_second.name AS second_sort,
    view_cargofile_third.name AS sort_name,
    view_cargofile_third.branch_id AS branch_id
FROM 
    uctoo_lvhuan.view_cargofile_third
JOIN 
    uctoo_lvhuan.view_cargofile_second ON view_cargofile_third.parent_id = view_cargofile_second.id
JOIN 
    uctoo_lvhuan.view_cargofile_first ON view_cargofile_second.parent_id = view_cargofile_first.id;
...
CREATE VIEW view_purchase_expense_collect AS 
SELECT 
...
CREATE VIEW view_reimbursement_expense_collect AS 
SELECT 
...
CREATE VIEW view_sort_expense_collect AS 
SELECT 
